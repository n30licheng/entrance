...
*	@author: 	licheng(80856841@qq.com)
*   @datetime:  2020-7-6
*   @history:   v1.0 first edition(qt5.13.2)
*/
#include "Qss.h"
...
QssTitleBar::QssTitleBar(QWidget *parent , QTitleBar_Type type/* = QTitleBar_Type_Window*/)
	: QWidget(parent),m_maxOrRestore(false),m_pressed(false),m_type(type)
{
    ...
    m_titlebarTitle->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred); //这种方式就不用水平占位
	/** 将上面的元素加入一个水平布局 */
    ...
    connect(m_closeBtn, SIGNAL(clicked()), this, SLOT(btn_close_slot()));
...
QssTitleBar::~QssTitleBar()
{
	//qDebug("=========QTitleBar-0x%x:deleteed!!========", this);
}

void QssTitleBar::setTitle( QString title )
{
	m_titlebarTitle->setText(title);
}

void QssTitleBar::setIcon( QIcon icon)
{
	m_iconBtn->setIcon(icon);
}

void QssTitleBar::setTitleCenter(bool t)
{
    m_titlebarTitle->setAlignment(Qt::AlignHCenter|Qt::AlignVCenter);
}

void QssTitleBar::setMaxOrRestore( bool val)
{
...
bool QssTitleBar::getMaxOrRestore()
{
    return m_maxOrRestore;
}

/** m_maxNormal为true表示窗口已最大化显示 */
void QssTitleBar::onMaxOrRestore()
{
	...
	if (m_maxOrRestore)
	{
        ...
        parentWidget()->setStyleSheet("QFrame#window{border-width:2px;}");
	...
	else 
	{
        ...
        parentWidget()->setStyleSheet("QFrame#window{border-width:0px;}");
...
void QssTitleBar::aboutMeslots()
{
    //qDebug()<<"------>>>";
    emit aboutMe();
}

void QssTitleBar::btn_close_slot()
{
    emit quit();
}
...
void QssTitleBar::paintEvent(QPaintEvent *)
{
	QStyleOption opt;
	opt.init(this);
	QPainter p(this);
...
void QssTitleBar::mouseMoveEvent( QMouseEvent * ev )
{
	if (m_maxOrRestore)
		return;
	if (!m_pressed)
...
void QssTitleBar::mousePressEvent( QMouseEvent * ev )
{
	m_pressed = true;
...
void QssTitleBar::mouseReleaseEvent( QMouseEvent* ev )
{
	m_pressed = false;
}

/** 中途截取标题栏双击消息和标题栏图标双击消息，分别用于处理最大最小化切换和双击标题栏图标关闭窗口 */
bool QssTitleBar::eventFilter( QObject * obj, QEvent * ev )
{
	if (obj == this)
	{
...
: QMainWindow(parent, flags),m_mousePressedInBoundy(false)
{
    ...
    m_titleBar = new QssTitleBar(m_frame);
	m_titleBar->installEventFilter(this);

    //css background: transparent;没有设置，这个会失效
   ...
   // connect(m_titleBar,&QssTitleBar::windowMaximized,this,&QssMainWindow::showFullScreen);
    //connect(m_titleBar,&QssTitleBar::quit,this)
	/** 动画效果 */
...
void QssMainWindow::setTitleCenter(bool t)
{
    m_titleBar->setTitleCenter(t);
}
...
void QssMainWindow::resizeEvent(QResizeEvent *event)
{
//    if(m_titleBar->getMaxOrRestore()){
//         m_rcValid =QGuiApplication::primaryScreen()->availableGeometry();
//         m_frame->setGeometry(m_rcValid);
//         m_frame->setStyleSheet("QFrame#window{border-width:0px;}");
//         return;
//     }
//    if(m_titleBar->getMaxOrRestore())
//        event->ignore();
...
QssTitleBar *QssMainWindow::get_titleBar()
{
    return m_titleBar;
}

QFrame *QssMainWindow::get_frame()
{
    return m_frame;
}
	...
	m_parent(parent)
{
    ...
    m_titleBar = new QssTitleBar(m_frame, QssTitleBar::QTitleBar_Type_Dialog);
	m_titleBar->installEventFilter(this);
...
:QMessageBox(icon, title, text, buttons, 0, flags),m_parent(parent)
{
    ...
    m_titleBar = new QssTitleBar(m_frame, QssTitleBar::QTitleBar_Type_MessageBox);
	m_titleBar->installEventFilter(this);
